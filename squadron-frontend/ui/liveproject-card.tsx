...
export const StyledIcon = styled(Icon)<StyledIconProps>`
  background-image: url(${(props) => {
    if (props.score >= 4.0 && props.score <= 5.0)
      return "/images/image_smile.png";
    if (props.score >= 3.0 && props.score < 4.0)
      return "/images/image_nofeeling.png";
...
const BalanceBox: React.FC<BalanceBoxProps> = ({
  ...
  return (
    <StyledBalanceBox>
      <TimeIcon time={time}>{time}h</TimeIcon>
      <BalanceInfo>
        <BalanceTypography>
        ...
        <Percentage isActive={time !== 0}>
          {time !== 0 && <img src="/icon/arrow-up.svg" alt="Increase" />}
...
export const SatisfactionBox: React.FC<{
  isInteractive?: boolean;
  score: number;
}> = ({ isInteractive = true, score, children }) => {
  ...
  return (
    <StyledCard>
      <StyledCardHeader
        ...
        subheader={
          <StyledCardMemberCount>{memberCount} Members</StyledCardMemberCount>
        }
      />
      <StyledCardContent>
        <DateTypography>Feb 01 - 07</DateTypography>
        <Box display="flex" gap="1rem" width="100%" height="100%">
          <BalanceBox
            time={balanceInfo.time}
